...
void Process_MSG() {  //byte Message[], byte Trailer[],int lenh) {
  ...
  ////@////p//SerialPtLnDebug(checkSum, HEX);
  for (int i=0; i<=bytePOS;i++){
    //p//SerialPtDebug(String(Message[i])+" ");
  }
  //p//SerialPtLnDebug();
  uint16_t combined = (Trailer[0] << 8) + Trailer[1];
  //@////p//SerialPtLnDebug(String( Message[1]) + "   CheckSumCal:" + String(checkSum) + "   CheckSumComb:" + String(combined) + "   Trailer_0:" + String(Trailer[0]) + "   Trailer_1:" + String(Trailer[1]));
  if (checkSum == combined) {
    ...
    while (MessagePOS < Message[0] - 3) {
      ...
      Run(FunctionID);
      //p//SerialPtLnDebug("Psa: " + String(millis() - Serial_MSG) + " ms");
    }
